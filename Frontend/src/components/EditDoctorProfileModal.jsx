...
const EditDoctorProfileModal = ({ isOpen, onClose, profile }) => {
  ...
  const handleUpdateDoctor = async (e) => {
    ...
    try {
      const res = await apiRequest.patch("/doctor/update", formData);
      ...
      <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100">
        <div className="px-8 py-6 bg-indigo-600  flex items-center justify-between text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold">Edit Professional Profile</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          <div className="mt-8 flex gap-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-4 text-sm font-bold text-slate-500 hover:text-slate-700 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-all"
            >
              Cancel
            </button>
            <button
